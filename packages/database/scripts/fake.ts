...
import { NUM_ADMINS, NUM_INDICATORS, NUM_PROFESSIONALS, NUM_REFERRALS } from './config'
import type {
  Attendance,
  AttendanceStatus,
  Commission,
...
export const fakeReferrals = {
  ...
  initialize() {
    this.admins = Array.from({ length: NUM_ADMINS }).map((_, i) => ({
      id: faker.string.uuid(),
      name: `Admin ${i + 1}`,
      email: `me+indicasaude+admin${i + 1}@rodrigo3d.com`,
    ...
    for (let i = 0; i < NUM_REFERRALS; i++) {
      const indicator = faker.helpers.arrayElement(this.indicators)
      const professional = faker.helpers.arrayElement(this.professionals)
